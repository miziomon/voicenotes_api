/**
 * ==============================================
 * Servizio Supabase Proxy V2 - Transparent Forwarding
 * ==============================================
 *
 * Versione 2 del proxy - Copia iniziale per modifiche future.
 *
...
logger.info(`[SupabaseProxyV2] Proxy V2 configurato per: ${SUPABASE_URL}`);

// ============================================
// CONFIGURAZIONE LOGGER DEDICATO PER IL PROXY V2
// ============================================
...
let proxyLoggerV2 = logger; // Default: usa il logger principale
...
if (!isVercel && !isProduction) {
    try {
        ...
        // Creiamo un logger dedicato per il proxy V2 con un file separato
        proxyLoggerV2 = winston.createLogger({
            level: 'debug',
            ...
            transports: [
                // File dedicato per il proxy V2
                new DailyRotateFile({
                    filename: path.join(logsDirectory, 'supabase-proxy-v2-%DATE%.log'),
        ...
        logger.info('[SupabaseProxyV2] Logger dedicato per proxy V2 creato con successo');
    } catch (error) {
        logger.warn(`[SupabaseProxyV2] Impossibile creare logger dedicato: ${error.message}. Verrà usato il logger principale.`);
    }
}
 ...
 */
const forwardToSupabaseV2 = async (req) => {
    ...
    try {
        ...
        if (isGatewayMode) {
            ...
            // Rimuoviamo il prefisso /supabase-proxy dal path (siamo già dentro /v2)
            ...
            proxyLoggerV2.info(`🔄 Modalità Gateway rilevata: ${method} ${targetPath}`);
        } else {
        ...
        proxyLoggerV2.info(`➡️  Forwarding ${method} ${targetPath} → Supabase`);
        proxyLoggerV2.debug(`URL completo: ${url.toString()}`);
        ...
        const supabaseHeaders = {
            ...
            ...filterClientHeadersV2(headers)
        };

        proxyLoggerV2.debug(`Headers preparati: ${JSON.stringify(Object.keys(supabaseHeaders))}`);
        ...
        if (body && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method.toUpperCase())) {
            ...
            proxyLoggerV2.debug(`Body incluso (${fetchOptions.body.length} caratteri)`);
        }
        ...
        proxyLoggerV2.debug(`🚀 Invio richiesta a Supabase...`);
        ...
        const responseHeaders = extractRelevantHeadersV2(response.headers);
        ...
        if (response.ok) {
            proxyLoggerV2.info(`✅ Risposta OK - Status: ${response.status}, Durata: ${duration}ms`);
        } else {
            proxyLoggerV2.warn(`⚠️  Risposta Errore - Status: ${response.status}, Durata: ${duration}ms`);
            proxyLoggerV2.warn(`Errore Supabase: ${JSON.stringify(responseData).substring(0, 200)}`);
        }
        ...
        proxyLoggerV2.error(`❌ Errore durante forwarding a Supabase: ${error.message}`);
        proxyLoggerV2.error(`Stack trace: ${error.stack}`);
 ...
 */
const filterClientHeadersV2 = (clientHeaders) => {
    const filtered = {};
 ...
 */
const extractRelevantHeadersV2 = (responseHeaders) => {
    const relevant = {};
...
module.exports = {
    ...
    filterClientHeadersV2,
    extractRelevantHeadersV2
};
